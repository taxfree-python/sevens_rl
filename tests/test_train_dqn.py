...
def test_train_episode_completes(env, agents):
    ...
    stats = train_episode(env, agents, logger, training_agent="player_0")
...
def test_evaluate_episode_completes(env, agents):
    ...
    stats = evaluate_episode(env, agents, training_agent="player_0")
...
def test_evaluate_episode_uses_zero_epsilon(env, agents):
    ...
    # Run evaluation
    evaluate_episode(env, agents, training_agent="player_0")
...
def test_train_episode_stores_experiences(env, agents):
    ...
    # Train one episode
    train_episode(env, agents, logger, training_agent="player_0")
...
def test_train_and_evaluate_integration(env, agents):
    ...
    # Evaluate
    eval_stats = evaluate_episode(env, agents, training_agent="player_0")
...
def test_different_training_agents(env):
    ...
    stats = train_episode(env, agents, logger, training_agent="player_1")
...
def test_training_session_smoke_test(env, agents):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for episode in range(1, num_episodes + 1):
            # Train
            stats = train_episode(env, agents, logger, training_agent="player_0")
            training_rewards.append(stats["training_agent_reward"])
